...
// Mock event data (simulating database)
const mockEvents = [
    {
        event_id: 1,
...
function animateStats() {
    const stats = [
        { id: 'totalEvents', target: 127, duration: 2000 },
        { id: 'activeEvents', target: 23, duration: 1500 },
...
function handleCostFilterChange() {
    ...
    AppState.currentFilters.costRange = { min, max };
    applyFilters();
}
    ...
    setTimeout(() => {
        ...
        if (AppState.currentFilters.dateRange.start || AppState.currentFilters.dateRange.end) {
            filtered = filtered.filter(event => {
                ...
                const startDate = AppState.currentFilters.dateRange.start;
                const endDate = AppState.currentFilters.dateRange.end;
...
function updateActiveFiltersDisplay() {
    ...
    if (AppState.currentFilters.dateRange.start || AppState.currentFilters.dateRange.end) {
        const dateRange = `${AppState.currentFilters.dateRange.start?.toDateString() || 'Any'} - ${AppState.currentFilters.dateRange.end?.toDateString() || 'Any'}`;
